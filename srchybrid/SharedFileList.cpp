...
// ==> Automatic shared files updater [MoNKi] - Stulle
#ifdef ASFU
// We need this for SlugFillers shareSubdir
void CSharedFileList::FindSubDirs()
{
	CString tempDir;
	CString ltempDir;
	...
	{
		// free final list
		thePrefs.tempsharedsubdir_list.RemoveAll();

		// get subdirs from current shared with subdirs dir
		GetSubDirs(l_sAddedWithSubdir.GetAt(pos));

		// put subdirs into final list
		pos2 = thePrefs.tempsharedsubdir_list.GetHeadPosition();
		while(pos2)
		{
			tempDir = thePrefs.tempsharedsubdir_list.GetNext(pos2);

			if(thePrefs.allsharedsubdir_list.Find(tempDir) == NULL)
				thePrefs.allsharedsubdir_list.AddTail(tempDir);
		}
	}
}

void CSharedFileList::GetSubDirs(const CString& rstrDirectory)
{
	thePrefs.tempsharedsubdir_list.AddHead(rstrDirectory);
	...
	while (!end)
	{
		...
		if (ff.IsDots() || ff.IsSystem() || ff.IsTemporary())
			continue; // skip
		if (ff.IsDirectory())
			GetSubDirs(ff.GetFilePath().GetBuffer());
	}
	ff.Close();
}
#endif
